...
import { loginSchema, registerSchema, setRoleSchema } from "../validators/AuthValidator"
...
export function createAuthController(): Router{
  ...
  router.get("/users", requireRole("ADMIN"), async (req, res) => {
    ...
    res.json({ users: users.map(u => ({ id: u._id.toString(), email: u.email, role: u.role, createdAt: u.createdAt })) })
  })
